...
def crop(input_image, real_image, feature):
    ...
    feature = tf.image.central_crop(feature, central_fraction = 256/448 )
    ...
    feature = tf.image.resize(feature, [256, 256],
                                method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
    return cropped_input, cropped_real, feature
...
@tf.function()
def preprocessing(input_image, real_image, feature):
...
def load_image_train(input_image, real_image, feature):
    input_image, real_image, feature = preprocessing(input_image, real_image, feature)
...
def Generator():
    ...
    x = tf.keras.layers.concatenate([inputs, features])  # (bs, 256, 256, 4)
...
def generate_images(model, test_input, tar, feature):
  ...
  feat_3d = np.stack((plot_feat, plot_feat, plot_feat), axis = 2)
  
  out = np.hstack((test_input, tar_3d, pred_3d, feat_3d))
  cv2.imwrite(input_directory+'progress/out_epoch_'+str(trained_epoch)+'.jpg', 255*(out))
